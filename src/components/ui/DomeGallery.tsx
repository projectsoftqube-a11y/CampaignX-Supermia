...
/* --------------------------------------------------------------------------
   DomeGallery — React Bits
   https://reactbits.dev
   Adapted for Next.js: added "use client" directive.
   -------------------------------------------------------------------------- */
...
const getDataNumber = (el: HTMLElement, name: string, fallback: number) => {
  const attr =
    (el.dataset as Record<string, string | undefined>)[name] ??
...
function buildItems(pool: ImageInput[], seg: number): Tile[] {
...
export default function DomeGallery({
  ...
  /* Auto-rotation: a gentle continuous spin that pauses when the user
     is dragging or has an image enlarged. */
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const dt = now - prev;
      prev = now;
      // Pause while dragging, during inertia, or when an image is open
      if (!draggingRef.current && !inertiaRAF.current && !focusedElRef.current) {
        const nextY = wrapAngleSigned(
          rotationRef.current.y + autoRotateSpeed * (dt / 16)
        );
